...
public class UsePassesJobConfig {
    ...
    public ItemProcessor<Booking, Booking> usePassesItemProcessor() {
        return bookingEntity -> {
            // 이용권 잔여 횟수는 차감합니다.
            Pass passEntity = bookingEntity.getPass();
            passEntity.setRemainingCount(passEntity.getRemainingCount() - 1);
            bookingEntity.setPass(passEntity);
    ...
    public ItemWriter<Booking> usePassesItemWriter() {
        return bookingEntities -> {
            for (Booking bookingEntity : bookingEntities) {
                // 잔여 횟수를 업데이트 합니다.
                int updatedCount = passRepository.updateRemainingCount(bookingEntity.getPassSeq(), bookingEntity.getPass().getRemainingCount());
